...
$ses = new Session($db);
    ...
    $err_msg = "";
    $ses->require_unlogined_session();//ログイン済みであればindex.phpに遷移

    $username = (isset($_POST["username"]) === true)?$_POST["username"]: "";
    $password = (isset($_POST["password"]) === true)?$_POST["password"]: "";
    if ($username !== "" && $password !== ""){
        $data = $db->select("account", "id, username, password", "username = '$username'");
    // var_dump($data);
...
<body>
    ...
    <header><h3>ログイン</h3></header>
        ...
        <a class="btn btn-primary" href="./regist.php">新規登録</a>
        <a href="./admin_login.php">管理者ページ</a>
    </div>
